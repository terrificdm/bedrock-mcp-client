...
if 'tool_server_map' not in st.session_state:
    st.session_state.tool_server_map = {}
if 'tool_use_history' not in st.session_state:
    st.session_state.tool_use_history = {}
# Store the current config path to detect changes
if 'current_config_path' not in st.session_state:
    st.session_state.current_config_path = ""
# Store the current region to detect changes
...
config_path = st.sidebar.text_input("MCP Configuration Path", value="mcp_config.json")
...
if 'current_model_id' not in st.session_state:
    st.session_state.current_model_id = "Anthropic Claude-3.5-Sonnet-v2"
...
region_name = st.sidebar.selectbox("AWS Region", options=region_options, index=0)
...
# Helper function to start MCP servers
def start_mcp_servers(config_path):
    try:
        # Load configuration
...
# Helper function to get tools from MCP servers using asyncio
def get_tools_from_servers():
    tools = []
    tool_server_map = {}
...
def call_tool(tool_name, tool_args, server_name):
    try:
        ...
        return {"text": f"Error calling tool {tool_name}: {str(e)}"}
...
if not st.session_state.connected:
    with st.spinner("Connecting to MCP servers..."):
        ...
        # Start MCP servers
        if start_mcp_servers(config_path):
            # Get tools from servers
            tools, tool_server_map, server_tools_map = get_tools_from_servers()
...
def handle_tool_use(message, messages, tools):
    for content_item in message['content']:
        if 'toolUse' in content_item:
            ...
            result = call_tool(tool_name, tool_args, server_name)
